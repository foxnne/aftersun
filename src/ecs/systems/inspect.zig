                ...
                if (inspect_tile) |*mouse_tile| {
                    ...
                    if (inspect_target != 0 and ecs.is_alive(world, inspect_target)) {
                        if (ecs.get(world, inspect_target, components.Position)) |target_tile_position| {
                            ...
                            const index: usize = std.math.clamp(@as(usize, @intFromFloat(@as(f32, @floatFromInt(examine_text.len)) * if (inspect_time >= 0.0) inspect_time else 0.0)), 1, examine_text.len);
                            ...
                            imgui.pushStyleColorImVec4(imgui.Col_ButtonHovered, .{ .x = im_text_color.x, .y = im_text_color.y, .z = im_text_color.z, .w = inspect_time });
                            ...
                            imgui.pushStyleColorImVec4(imgui.Col_TitleBg, im_text_color);
                            imgui.pushStyleColorImVec4(imgui.Col_TitleBgActive, im_text_color);
                            imgui.pushStyleColorImVec4(imgui.Col_Border, im_text_color);

                            imgui.pushStyleVar(imgui.StyleVar_WindowBorderSize, 1.0);
                            defer imgui.popStyleVar();

                            defer imgui.popStyleColorEx(8);

                            imgui.setNextWindowPos(window_pos, imgui.Cond_Always);
                            const flags: imgui.WindowFlags = imgui.WindowFlags_AlwaysAutoResize | imgui.WindowFlags_NoCollapse | imgui.WindowFlags_NoNav;
                            ...
                            if (show_choice_dialog and inspect_time > 0.0) {
...
fn drawWindowEdges(draw_list: *imgui.DrawList, color: game.math.Color) void {
    ...
    const zoom_text = std.fmt.allocPrintZ(game.state.allocator, "zoom {d}x", .{game.state.camera.zoom}) catch unreachable;
    defer game.state.allocator.free(zoom_text);

    draw_list.addText(.{ .x = window_pos.x - offset + 5.0, .y = window_pos.y + window_height + offset - imgui.getTextLineHeight() - 3.0 }, color.toU32(), zoom_text);
