            ...
            if (ecs.field(it, components.Position, 2)) |player_positions| {
                ...
                if (target_entity) |target| {
                    if (target == inspect_target) {
                        if (inspect_time < 1.0) {
                            inspect_time += it.delta_time * 2.0;
                        } else {
                            inspect_time = 1.0;
                        }
                    } else {
                    ...
                    const index: usize = std.math.clamp(@as(usize, @intFromFloat(@as(f32, @floatFromInt(examine_text.len - 1)) * inspect_time)), 1, examine_text.len - 1);
                    ...
                    if (imgui.begin("InspectDialog", null, flags)) {
                        ...
                        if (imgui.getForegroundDrawList()) |draw_list| {
                            ...
                            //     .{ .x = @floor(window_pos.x + width.x / 2.0 - 5.0), .y = @floor(window_pos.y + imgui.getWindowHeight()) },
                            //     .{ .x = @floor(window_pos.x + width.x / 2.0), .y = @floor(window_pos.y + imgui.getWindowHeight() + 8.0) },
                            //     .{ .x = @floor(window_pos.x + 5.0 + width.x / 2.0), .y = @floor(window_pos.y + imgui.getWindowHeight()) },
                            //     game.settings.colors.background.toU32(),
                            // );
                        }
