...
pub fn system() flecs.EcsSystemDesc {
    ...
    desc.query.filter.terms[2] = std.mem.zeroInit(flecs.EcsTerm, .{ .id = flecs.ecs_pair(components.RequestOther, components.Stack), .oper = flecs.EcsOperKind.ecs_optional });
        ...
        while (i < it.count) : (i += 1) {
            ...
            if (flecs.ecs_field(it, components.Stack, 1)) |stacks| {
                if (flecs.ecs_field(it, components.Stack, 2)) |requests| {
                    ...
                    if (flecs.ecs_field(it, components.RequestOther, 3)) |request_others| {
                        ...
                        flecs.ecs_remove_pair(world, entity, components.RequestOther, components.Stack);
                    }
