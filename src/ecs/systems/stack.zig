...
pub fn system() ecs.system_desc_t {
    ...
    desc.query.filter.terms[0] = std.mem.zeroInit(ecs.term_t, .{ .id = ecs.id(components.Stack) });
    desc.query.filter.terms[1] = std.mem.zeroInit(ecs.term_t, .{ .id = ecs.pair(ecs.id(components.Request), ecs.id(components.Stack)) });
    desc.query.filter.terms[2] = std.mem.zeroInit(ecs.term_t, .{ .id = ecs.pair(ecs.id(components.RequestZeroOther), ecs.id(components.Stack)), .oper = ecs.oper_kind_t.Optional });
    desc.query.filter.terms[3] = std.mem.zeroInit(ecs.term_t, .{ .id = ecs.pair(ecs.id(components.Cooldown), ecs.id(components.Movement)), .oper = ecs.oper_kind_t.Not });
