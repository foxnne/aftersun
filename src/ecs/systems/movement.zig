...
pub fn system(world: *ecs.world_t) ecs.system_desc_t {
    ...
    desc.query.filter.terms[1] = .{ .id = ecs.pair(ecs.id(components.Request), ecs.id(components.Movement)) };
    desc.query.filter.terms[2] = .{ .id = ecs.pair(ecs.id(components.Cooldown), ecs.id(components.Movement)), .oper = ecs.oper_kind_t.Optional };
    desc.run = run;
    ...
    while (ecs.iter_next(it)) {
        ...
        while (i < it.count()) : (i += 1) {
            ...
            if (ecs.field(it, components.Position, 1)) |positions| {
                if (ecs.field(it, components.Movement, 2)) |movements| {
                    ...
                    if (positions[i].tile.x != movements[i].end.x or positions[i].tile.y != movements[i].end.y or positions[i].tile.z != movements[i].end.z) {
                        if (entity == game.state.entities.player) {
                            const player_cell = positions[i].tile.toCell();
                            var next_cell = movements[i].end.toCell();

                            if (player_cell.x != next_cell.x or player_cell.y != next_cell.y) {
                                if (player_cell.y > next_cell.y and positions[i].tile.y > game.settings.cell_size + 1) {
                        ...
                        positions[i].tile.x = movements[i].end.x;
                        positions[i].tile.y = movements[i].end.y;
                        positions[i].tile.z = movements[i].end.z;
                        positions[i].tile.count();

                        // Set modified so that observers are triggered.
                        ecs.modified_id(world, entity, ecs.id(components.Position));
                    }
                    ...
                    if (ecs.field(it, components.Cooldown, 3)) |cooldowns| {
                        if (!instant) {
                            ...
                            const start_position = movements[i].start.toPosition(.position).toF32x4();
                            const end_position = movements[i].end.toPosition(.position).toF32x4();
